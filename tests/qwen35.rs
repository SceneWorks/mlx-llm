...
//! gated-full-attention decoder (stories sc-7627…sc-7629). `#[ignore]` — needs the 27B snapshot:
//!
//! ```text
...
//! ```
//!
//! These are the acceptance gate for sc-7629: dispatch (family `qwen3_5`), coherent greedy text on
//! real weights, and the thinking / no-think split driven by the model's own chat template.
...
#[test]
#[ignore = "needs the Qwen3.6-27B snapshot via MLX_LLM_QWEN35_MODEL"]
fn qwen35_dispatch_and_coherent_text() {
    let p = LlamaProvider::load(&LoadSpec::dense(model_dir())).expect("load qwen3.6-27b");
